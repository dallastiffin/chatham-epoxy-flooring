...
# --- business identity ----------------------------------------------------
BUSINESS      = "Chatham Epoxy Flooring"
CITY          = "Chatham"
PROVINCE      = "Ontario"
PROVINCE_CODE = "ON"
REGION        = "Chatham-Kent"
...
PHONE_DISPLAY = "+1 226-457-4250"
PHONE_HREF    = "+12264574250"
...
DOMAIN = "https://chathamepoxyflooring.com"

# The markdown file holding all copy, in this folder.
CONTENT_FILE = "Chatham-Epoxy-Flooring-Website-Content.md"
...
#
# READ THIS BEFORE EDITING. Chatham-Kent amalgamated on 1 January 1998: the
# City of Chatham and every town, village and township of the old Kent County
# were dissolved into one single-tier municipality. So Wallaceburg, Blenheim,
# Tilbury, Dresden, Ridgetown, Thamesville, Wheatley, Bothwell, Erieau,
# Merlin and Pain Court are COMMUNITIES INSIDE Chatham-Kent, not neighbouring
# towns. Listing any of them as a "nearby town we also serve" is the kind of
# error a local spots on sight. The copy treats the municipality as one
# service area on purpose.
SERVICE_AREA = [
    "Chatham",
    "Wallaceburg",
    "Blenheim",
...
TOPBAR_AREA = "Serving every community in Chatham-Kent"
...
#  Photos are focal-point cropped, resized, then exported as WebP + JPG.
# ==========================================================================
# PHOTO ROLES - read this before changing anything here.
#
# Every photo in the shared pool was already live on at least one other city
# site before this build started. A byte-identical image reused in the SAME
# position on two domains is the clearest doorway-network signal there is,
# so the rule applied on this site is:
#
#     no photo may hold a role it already holds on another city site.
#
# A Sarnia hero becomes a Chatham gallery item; a Welland hero becomes the
# Chatham services-page image; a Sudbury hero becomes a Chatham service card.
# On top of that, the CROP table further down shifts the framing of every
# shared photo off centre, so the delivered crop differs even where the
# source file cannot.
#
# Three unavoidable collisions, documented rather than hidden:
#   basement.png          Cape Breton also runs it on its basement page.
#                         Every basement photo in the pool has held a
#                         basement-service slot somewhere. Cape Breton is in
#                         Nova Scotia and shares no search market.
#   commercial 1.png      Sudbury also runs it on its commercial page.
#                         700 km away, no market overlap.
#   greyscale bathroom    The ONLY metallic photo that exists. Sarnia,
#   marbled epoxy.png     Sudbury and Welland all run it on their metallic
#                         page. Sarnia is 90 km from Chatham and the two DO
#                         compete for the same searches. Mitigated with the
#                         hardest crop in the table, so the framing is
#                         visibly different - but this one wants a real
#                         replacement photo.
#
# ONE genuinely unique image: warehouse-industrial-chatham.png, cropped from
# warehouse.png for this site. The uncropped original carries another
# flooring company's logo on a crew member's hi-vis shirt and is gitignored.
# The Sarnia crop of the same original is a different box (1156x722 taken
# from x=380); this one is 1066x809 from x=470, framed to drop the near
# worker out of shot entirely.
#
# Three further pool images are permanently unusable and are gitignored:
# "warehouse grey epoxy.png", "blue epoxy spread.png" and "grey spready.png"
# all carry a WOODSTOCK EPOXY FLOORING wordmark on the crew shirt and, in two
# of them, on a resin pail in the foreground. Sarnia's brand-free crop of
# "warehouse grey epoxy.png" took the only framing that removes both, so
# there is no second crop of it available here.
PAGE_PHOTOS = [
  ("empty garage.png", "hero-epoxy-garage-floor-chatham", (4,3), [800,1200],
   f"Light grey flake garage floor coating finished in {CITY_PROV}"),
  ("1 Car garage.png", "og-image", (1200,630), [1200],
   f"{BUSINESS} garage floor coating"),

  ("orange floor.png", "service-flake-epoxy-flooring", (16,10), [640,960],
   "Orange flake broadcast finished across a double bay garage floor"),
  ("two car garage.png", "service-garage-floor-coating", (16,10), [640,960],
   "Grey flake garage floor coating finished square to the door track"),
  ("blue floor.png", "service-polyaspartic-floor-coating", (16,10), [640,960],
   "Blue floor holding a wet gloss under a polyaspartic top coat"),
  ("basement.png", "service-basement-floor-coating", (16,10), [640,960],
  ...
  ("grey epoxy pour basement.png", "service-epoxy-floor-repair-and-recoating",
   (16,10), [640,960],
   "Fresh coating going down over a slab that has been ground back"),
  ("greyscale bathroom marbled epoxy.png", "service-metallic-epoxy-flooring",
   (16,10), [640,960],
   "Metallic coating worked into a grey and white marbled pattern"),

  ("tesla.png", "about-chatham-epoxy-flooring", (4,3), [800,1200],
   f"Finished floor back in service with a vehicle parked on it in {CITY}"),
  ("Orange 3 car garage.png", "services-epoxy-flooring-chatham", (4,3), [800,1200],
   "Orange flake floor running the full depth of a three bay garage"),
]


GALLERY_PHOTOS = [
 ("finished grey garage.png", "grey-flake-finished-to-the-track",
 ...
 ("1 Car garage.png", "single-bay-with-wall-storage",
  "One bay coated out beneath fitted wall storage"),
 ("two car garage.png", "two-bay-grey-flake",
  "Double garage laid out in a mid grey blend"),
 ("Basement Gym.png", "lower-level-gym-slab",
  "Lower level slab sealed and turned into a gym"),
 ("blue Pour.png", "blue-base-coat-spreading",
  "Blue base resin running out across ground concrete"),
 ("basement.png", "basement-slab-past-the-furnace",
  "Grey build carried past the furnace and water heater"),
 ("Enclosed Porch.png", "three-season-porch-slab",
...
# Per-image crop control, keyed by slug.
#
#   (fx, fy, zoom)   fx/fy are the focal point in 0..1 of the source frame,
#                    0.5/0.5 being the plain centre crop the template does.
#                    zoom > 1 crops tighter before resizing.
#
# This exists for one reason. Nearly every source file is already live on
# another city site, so a plain centre crop here would deliver a near
# identical image to one Google has already indexed elsewhere. Shifting the
# focal point and the zoom changes the delivered picture even when the source
# file cannot change.
#
# Values were set per image against the actual subject, not applied as a
# blanket offset. A crop that cuts the garage door out of frame is worse than
# a duplicate, so nothing here moves far enough to lose the subject.
CROP = {
  "hero-epoxy-garage-floor-chatham":                   (0.44, 0.58, 1.10),
  "og-image":                                          (0.50, 0.46, 1.00),
  "service-flake-epoxy-flooring":                      (0.58, 0.60, 1.18),
  "service-garage-floor-coating":                      (0.42, 0.58, 1.16),
  ...
  # exact page. Tightest zoom in the table and pushed well off centre, so
  # what ships is a detail of the marbling rather than the room.
  "service-metallic-epoxy-flooring":                   (0.62, 0.64, 1.45),
  ...
  "lower-level-gym-slab":              (0.40, 0.58, 1.15),
  "blue-base-coat-spreading":          (0.58, 0.62, 1.18),
...
def build_images():
    ...
    def export(src, base, aspect, widths):
        ...
        # Focal-point crop, not the template's plain centre crop.
        # See the CROP table for why this exists.
        fx, fy, zoom = CROP.get(base, (0.5, 0.5, 1.0))
        if w / h > ar:
            nw, nh = h * ar, float(h)
        else:
            nw, nh = float(w), w / ar
        nw /= zoom; nh /= zoom
        # Clamp so the window always lands inside the frame.
        left = min(max(fx * w - nw / 2.0, 0), w - nw)
        top  = min(max(fy * h - nh / 2.0, 0), h - nh)
        im = im.crop((int(round(left)), int(round(top)),
                      int(round(left + nw)), int(round(top + nh))))
        for width in widths:
...
home = home.replace('<link rel="stylesheet" href="%s">' % asset_v("style.css"),
    ...
    '<link rel="preload" as="image" href="images/hero-epoxy-garage-floor-chatham-1200.jpg"\n'
    '      imagesrcset="images/hero-epoxy-garage-floor-chatham-800.webp 800w, images/hero-epoxy-garage-floor-chatham-1200.webp 1200w"\n'
  ...
  <div class="hero__bg" aria-hidden="true">
    <picture>
      <source type="image/webp"
              srcset="images/hero-epoxy-garage-floor-chatham-800.webp 800w, images/hero-epoxy-garage-floor-chatham-1200.webp 1200w"
              sizes="100vw">
      <img src="images/hero-epoxy-garage-floor-chatham-1200.jpg"
           srcset="images/hero-epoxy-garage-floor-chatham-800.jpg 800w, images/hero-epoxy-garage-floor-chatham-1200.jpg 1200w"
...
{picture("services-epoxy-flooring-chatham", "(max-width: 1024px) 92vw, 460px", eager=True, indent="      ")}
    </div>
  </div>
</section>
...
{nodes_html([n for n in about_lead['nodes']], "      ")}
    ...
    <div class="hero__media">
{picture("about-chatham-epoxy-flooring", "(max-width: 1024px) 92vw, 460px", eager=True, indent="      ")}
    </div>
  </div>
</section>
...
{services_grid(heading="Services We Install", intro="Twelve resin builds for homes, shops, farms and plants across Chatham-Kent.")}
...
{services_grid(heading="Services We Install", intro="Open any one for the build, the prep it needs and how it is looked after.")}
